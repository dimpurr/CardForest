...
### 第一阶段：代码组织和结构优化
- [ ] 1.1 统一 GraphQL 查询管理
  - [ ] 1.1.1 创建统一的 GraphQL 目录结构
  - [ ] 1.1.2 整合所有查询和变更到相应文件
  - [ ] 1.1.3 创建共享片段减少重复代码
  - [ ] 1.1.4 配置 GraphQL Code Generator
- [ ] 1.2 统一类型定义
  - [ ] 1.2.1 创建共享类型库
  - [ ] 1.2.2 整合现有类型定义
  - [ ] 1.2.3 更新 tsconfig.json 启用严格类型检查
- [ ] 1.3 统一状态管理
  - [ ] 1.3.1 创建统一的 Jotai atoms 目录结构
  - [ ] 1.3.2 整合现有 atoms
  - [ ] 1.3.3 实现持久化状态（如需要）
...
- [ ] 2.1 统一认证流程
  ...
  - [ ] 2.1.3 统一用户对象格式
- [ ] 2.2 创建认证上下文
  - [ ] 2.2.1 实现 AuthContext
  - [ ] 2.2.2 创建认证钩子（useAuth）
  - [ ] 2.2.3 整合现有认证逻辑

### 第三阶段：组件和 API 封装
- [ ] 3.1 创建 GraphQL API 封装
  - [ ] 3.1.1 封装 Apollo Client 操作
  - [ ] 3.1.2 提供类型安全的 API 钩子
  - [ ] 3.1.3 统一错误处理
- [ ] 3.2 重构 UI 组件
  - [ ] 3.2.1 创建可复用的组件
  - [ ] 3.2.2 实现组件文档
